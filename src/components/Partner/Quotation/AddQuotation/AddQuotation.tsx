...
const AddQuotation = () => {
  ...
  useEffect(() => {
    if (!isAdd && leadId) {
      getQuotationByLeadIdService({ header, leadId })
        .then((leadDetails) => setViewQuotationDetails(leadDetails.data))
      ...
      getLeadByIdService({ header, leadId })
        .then((leadDetails) => {
          ...
          [
            ...
            if (leadDetails.data[docName]) {
              updatedDocuments.push({
                docName,
                file: imagePath + leadDetails.data[docName],
              });
            }
          });
  ...
  const handleFileInputChange = (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
    if (event.target.files && event.target.files[0]) {
      ...
      if (file.size > MAX_FILE_SIZE) {
        setErrorMessage("Image size should be <= 4MB.");
      } else {
        setErrorMessage("");
  
        const updatedDocuments = [...documents];
        updatedDocuments[index].file = URL.createObjectURL(file); // फाइल का URL स्टोर करें
        setDocuments(updatedDocuments);
      }
    }
  };  

  const handleChangeDocumentName = (newValue: { label: string; value: string } | null, index: number) => {
  ...
  const handleClickAddDocument = () => {
    ...
    setErrors([...errors, { docName: "", file: "" }]); // Error validation के लिए
  };
  ...
  const onSubmit = (quotationForm: any, form: any) => {
    ...
    documents.forEach((doc: Document) => {
            if (doc.file) {
              editFrom.append(doc.docName, doc.file);
            }
          });
  ...
  value={
    doc.docName
      ? { label: doc.docName, value: doc.docName }
      : null
  }
  options={
  ...
  isOptionEqualToValue={(option, value) => option.value === value.value} // सुनिश्चित करें कि चयनित मान सही बना रहे
  onChange={(e, newValue) => handleChangeDocumentName(newValue, index)}
  renderInput={(params) => (
    <TextField
      {...params}
