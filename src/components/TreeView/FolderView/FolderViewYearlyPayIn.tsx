...
import GetBrokerWithBalanceCompanyPaymentService from "../../../api/Dashboard/GetBrokerWithBalanceCompanyPayment/GetBrokerWithBalanceCompanyPaymentService";
      ...
      try {
        if (api === "year_payin") {
          if ("name" in node) {
            ...
            const res = await GetBrokerWithCompanyPaymentService({
              header,
              brokerId,
          ...
          if ("name" in node) {
            ...
            const brokerId = node.brokerId;
            const res = await 	GetBrokerWithBalanceCompanyPaymentService(
              {
                header,
                brokerId,
                category: selectedCategory,
        ...
        } else if (api === "year_payin_balance") {
          if ("name" in node) {
            if (node.children) {
              setChildrenData(node.children);
            }
        ...
        } else if (api === "year_left_dis_amount") {
          if ("name" in node) {
            if (node.children) {
              setChildrenData(node.children);
            }
            ...
            const brokerId = node.brokerId;
            const res = await GetBrokerWithLeftDistributedCompanyPaymentService(
              {
                header,
                brokerId,
                category: selectedCategory,
...
const FolderViewYearlyPayIn: React.FC<FolderLikeStructureForPayInProps> = ({
  data,
  api,
}) => {
  return (
...
export default FolderViewYearlyPayIn;
