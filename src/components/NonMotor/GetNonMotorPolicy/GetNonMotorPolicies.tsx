...
import { nonMotorPolicyAddPath } from "../../../sitemap";
...
const GetNonMotorPolicies = () => {
  ...
  const [nonMotorPolicies, setNonMotorPolicies] = useState<IViewPolicy[]>([]);
  ...
  const handleClickAddNonMotorPolicy = () => {
 ...
 useEffect(() => {
  ...
  const handleSaveCell = async (
    ...
    if (cell.column.id === "partnerName") {
      ...
      nonMotorPolicies[cell.row.index][key] = Number(value);
      ...
      nonMotorPolicies[cell.row.index]["payInODAmount"] =
        calculatedPayInODPercentage;
      policyForm.payInTPAmount = calculatedPayInTPPercentage;
      nonMotorPolicies[cell.row.index]["payInTPAmount"] =
      ...
      nonMotorPolicies[cell.row.index]["payInCommission"] = payInCommission;
      ...
      nonMotorPolicies[cell.row.index]["payOutODAmount"] =
        calculatedPayOutODPercentage;
        nonMotorPolicies[cell.row.index]["payOutTPAmount"] =
      ...
      nonMotorPolicies[cell.row.index]["payOutCommission"] = payOutCommission;
      // try {
      //   const newPayment = await editMotorPolicyPaymentService({
      //     header,
      //     policyPayment: policyForm,
      //   });
      //   nonMotorPolicies[cell.row.index][cell.column.id as keyof IViewPolicy] =
      //     value;
  ...
  //   () =>
  //     nonMotorPolicies?.map(
  //       (nonMotorPolicy: IViewPolicy) =>
  //         ({
  //           id: motorPolicy._id,
  ...
  const updateLoading = useCallback(async () => {
    setIsLoading(nonMotorPolicies.length >= 0 ? false : true);
  }, [nonMotorPolicies]);
  useEffect(() => {
    updateLoading();
  }, [updateLoading]);
  ...
  const handleClickDownloadDocuments = async (policy: IViewPolicy) => {
    ...
    if (policy.currentPolicy) {
      downloadFile(
        `${imagePath}${policy?.currentPolicy!}`,
        "currentPolicy"
      );
    }
    if (policy.previousPolicy) {
      downloadFile(
        `${imagePath}${policy?.previousPolicy!}`,
        "previousPolicy"
      );
    }
    if (policy.survey) {
        ...
        <Paper elevation={3} style={{ padding: 30 }}>
          ...
          userData.role.toLowerCase() === "account" ? ( */}
              <Button
                type="button"
                size="small"
                onClick={handleClickAddNonMotorPolicy}
